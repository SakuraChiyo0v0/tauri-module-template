...
一个与桌面底座源码完全分离的 `.mtp` 模块开发模板。它提供 Host SDK V3 类型、浏览器模拟宿主、模块隔离 SQLite、私有文件与用户授权程序示例、原生 Web Component、测试、单文件 ESM 构建和确定性打包。
...
浏览器预览用于快速检查页面、模块私有设置、主题订阅、日志、数据库和私有文件调用。SQLite 与私有文件由浏览器内存或 `localStorage` 模拟；注册表、真实进程、系统托盘和全局快捷键必须安装 `.mtp` 到 Tauri 底座验证。模块不得直接调用原始 Tauri invoke。
